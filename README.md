...
- audience - the value the IdP puts into the access token's aud claim. Out of the box an access token is accepted when its aud carries the client-id, or when the claim naming the client the token was issued to does - azp (OpenID Connect Core 1.0 §2), appid (Entra ID v1.0 tokens and AD FS) or client_id (RFC 7662), whichever of those the token carries first in that order of precedence. Only the most authoritative one present is consulted, so a token whose azp truthfully names another client is refused even when it also carries a client_id naming ownCloud - a shape a tenant can produce on a shared realm with a hardcoded-claim mapper. The second half of the rule exists because an access token's aud belongs to the resource server (RFC 9068 §3), so many IdPs never put the client-id there: Keycloak sends no aud at all unless an audience mapper is configured, Entra ID v1.0 tokens send the App ID URI, ADFS sends microsoft:identityserver:<identifier> (prefixing the application identifier from `Get-AdfsWebApiApplication`, an OpenID Connect application group, or `Get-AdfsRelyingPartyTrust`, a legacy WS-Federation or SAML trust, unless that identifier is already a URL). Set audience to declare what your IdP actually sends and it becomes the only accepted value, on the introspection path too: takes a string or a list of strings, replaces client-id rather than adding to it, and must match exactly, including case. That cuts both ways - a token issued to this client for some other resource is then rejected, which is the reason to set it, but a token issued to a different client of the same IdP is accepted when its aud names ownCloud, so pick a value only ownCloud's relying party can be issued for. Leaving it unset still refuses a token an attacker obtained for their own client and addressed at their own client; what it adds is a binding to the resource, so set it if your IdP issues tokens to ownCloud's client for more than one resource (RFC 8707, RFC 8693). It cannot keep another client's tokens out - a token whose aud names ownCloud is accepted whichever client requested it, with or without the key - which is a decision to make in the IdP. Independently of the audience, a token that labels its own type must label itself an access token: typ of Bearer (Keycloak) or at+jwt (RFC 9068 §2.1), token_use of access (AWS Cognito, which also accepts access_token); any of those values is accepted in either claim, but every label the token carries has to be one of them, so anything else is refused - which covers refresh, offline, back-channel logout, registration and ID tokens on providers that label them, and the generic JOSE media type jwt along with them: a provider that stamps that into the payload stamps it on every token it signs, refresh tokens included, so accepting it would switch the check off for precisely the provider it would be meant to accommodate. No setting relaxes this - the check runs before the audience is looked at and reads nothing from the configuration. A provider that puts no type claim in the payload is unaffected, so with Entra ID or ADFS an ID token still satisfies the default expectation - it is accepted for as long as the client-id is an accepted audience, and setting audience to anything else rejects it as a side effect. Do not set audience at all if your introspection response omits aud, since every opaque token would then be rejected. With exchange-token-mode-before-introspection the first entry that survives the usable-string filter is also what the token exchange asks the IdP for, so list the resource ownCloud should be given first and keep values that cannot be an audience out of the list
