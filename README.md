...
- audience - the value the IdP puts into the access token's aud claim - an access token is only accepted if this names ownCloud. Takes a string or a list of strings, and replaces client-id as the expected value rather than adding to it. Defaults to client-id, which is what most IdPs send; set it only if your IdP names the resource server instead, as ADFS does - it prefixes the application identifier from `Get-AdfsWebApiApplication` (an OpenID Connect application group) or `Get-AdfsRelyingPartyTrust` (a legacy WS-Federation or SAML trust) with `microsoft:identityserver:` unless that identifier is already a URL. The value must match exactly, including case. Setting it makes aud authoritative for opaque tokens too, which cuts both ways: a token issued to this client for some other resource is rejected, but a token issued to a different client of the same IdP is accepted when its aud names us - so pick a value only ownCloud's relying party can be issued for. Do not set it at all if your introspection response omits aud, since every opaque token would then be rejected. With exchange-token-mode-before-introspection the first entry is also what the token exchange asks the IdP for, so list the resource ownCloud should be given first
